...
export const initializeMockAdapter = () => {
  ...
  mock.onGet(pathToRegexp(apiRoutes.getPhoneDetail)).reply((config) => {
    if (!getUser(config)) {
      return [403];
    }

    const result = match<{ id: string }>(apiRoutes.getPhoneDetail, {
      decode: decodeURIComponent,
    })(config.url!);
    ...
    return [200, phoneDetailsMock.find((item) => item.id === +id)];
  });
